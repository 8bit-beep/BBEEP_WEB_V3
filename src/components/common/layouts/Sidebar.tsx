...
const Sidebar = () => {
    ...
    const {
        data: selfstudy,
        isLoading: selfstudyLoading,
        refetch: selfstudyRefetch,
    } = useGetAttendsByRoom(sidebarData, "WINTER_CAMP_SELF_STUDY");
    const {
        data: lecture,
        isLoading: lectureLoading,
        refetch: lectureRefetch,
    } = useGetAttendsByRoom(sidebarData, "WINTER_CAMP_LECTURE");
    const data = [...selfstudy, ...lecture];
    const isLoading = selfstudyLoading || lectureLoading;
    const refetch = async () => {
    ...
    const attendedCount = data.filter(
        (attend) => attend.statuses[statusIdx].status !== "NOT_ATTEND"
    ).length;
        ...
        <div className="w-full h-screen flex flex-col gap-3 bg-white pt-13 pb-36 px-4 z-10">
            ...
            <div className="w-full flex items-center gap-4">
                ...
                <div className="flex gap-1">
                    ...
                    {/* 새로고침 버튼 */}
                    <div
                        className="py-1 px-3 text-main bg-gray rounded-xl transition-all duration-[0.3s] text-xs cursor-pointer"
                        onClick={() => refetch()}
                    >
                        새로고침
                    </div>
