  ...
  return (
    <div className="w-full flex items-center h-16">
      <TableItemContent $flex={1}>
        {props.user.studentInfo.grade}
        {props.user.studentInfo.cls}
        {props.user.studentInfo.num < 10
          ? `0${props.user.studentInfo.num}`
          : props.user.studentInfo.num}
      </TableItemContent>
      <TableItemContent $flex={1}>{props.user.username}</TableItemContent>
      <TableItemContent $flex={2}>{parseRoomName(props.room)}</TableItemContent>
      ...
      {props.status === "WAITING" ? (
        ...
        <div className="h-9 flex items-center flex-[2] justify-center gap-2">
          ...
          <p
            ...
            style={{
              color: props.status === "APPROVED" ? COLOR.Main : COLOR.Red,
            }}>
